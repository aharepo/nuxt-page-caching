...
test("canonicalUrl stores canonical HTML for nocache renew requests", async () => {
  const html = createNuxtDataHtml({ path: "/about?nocache=1" });
  const { deps, operations } = createDeps({
    getCacheData: () => ({
      key: "page-key",
      expire: 60,
  ...
  const ctx = {
    event: createEvent("/about?nocache=1"),
    response: undefined,
  };
  const response = { body: html, statusCode: 200 };
  ...
  const write = operations.find((op) => op.action === "write");
  const stored = JSON.parse(write.value);
  assert.equal(getPayloadPath(extractNuxtDataPayload(stored.html)), "/about");
  assert.equal(
    getPayloadPath(extractNuxtDataPayload(response.html)),
    "/about?nocache=1"
  );
  assert.match(response.html, /"\\u002Fabout\?nocache=1"/);
  assert.match(stored.html, /"\\u002Fabout"/);
  assert.doesNotMatch(stored.html, /"\\u002Fabout\?nocache=1"/);
});
