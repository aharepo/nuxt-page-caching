...
function isDebugLogEnabled(options = {}) {
  const env = globalThis.process?.env || {};
  return (
    isDebugValueEnabled(options.debug) ||
    isDebugValueEnabled(options.nuxt3?.debug) ||
    isDebugValueEnabled(env.NUXT_PAGE_CACHE_DEBUG) ||
    isDebugValueEnabled(env.NUXT_PAGE_CACHE_SINGLE_FLIGHT_DEBUG)
  );
}

function debugLog(options, message, details = {}) {
  if (!isDebugLogEnabled(options)) return;

  console.info("[nuxt-page-caching]", message, details);
}
...
export async function handleRenderBefore(ctx, deps) {
  ...
  if (cacheData.renewCache) {
    debugLog(options, "renew cache requested; rendering", {
  ...
  try {
    ...
    if (pendingRender) {
      debugLog(options, "cache miss; waiting for in-flight render", {
        route: resolved.route,
        redisKey,
      });
      ...
      if (cachedObjectAfterPendingRender) {
        debugLog(options, "served from in-flight render cache", {
          route: resolved.route,
          redisKey,
        });
      ...
      debugLog(options, "in-flight render finished without cache; rendering", {
        route: resolved.route,
        redisKey,
      });
    }
    ...
    debugLog(options, "cache miss; rendering", {
      route: resolved.route,
      redisKey,
    });
  } catch (error) {
...
export async function handleRenderResponse(response, ctx, deps) {
  ...
  try {
    ...
    debugLog(options, "cache write", {
      route: state.route,
      redisKey: state.redisKey,
      expire: state.cacheData.expire,
    });
