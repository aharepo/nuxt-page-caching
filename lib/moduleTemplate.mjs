...
function getPluginDirectory(nuxt) {
  return path.resolve(
    nuxt.options.buildDir || path.join(nuxt.options.rootDir, ".nuxt"),
    "nuxt-page-caching"
  );
}

function toRelativeImportSpecifier(fromDir, targetPath) {
  let relativePath = toPosixPath(path.relative(fromDir, targetPath));
...
function resolveNuxtImportPath(importPath, nuxt) {
  ...
  if (importPath.startsWith("@/") || importPath.startsWith("~/")) {
    return toRelativeImportSpecifier(
      getPluginDirectory(nuxt),
      path.resolve(nuxt.options.srcDir, importPath.slice(2))
    );
  }
  ...
  if (path.isAbsolute(importPath)) {
    return toRelativeImportSpecifier(getPluginDirectory(nuxt), importPath);
  }
